...
void CStat::setRepartitionCallLength(char * P_listeStr)
{
  ...
  if(createIntegerTable(P_listeStr, &listeInteger, &sizeOfListe) == 1) {
    ...
    // TODO: ERROR("Could not create table for call length repartition '%s'\n", P_listeStr);
  }
  delete [] listeInteger;
  listeInteger = NULL;
}
  ...
  for (i = 0; i < nRtds(); i++) {
    if(createIntegerTable(P_listeStr, &listeInteger, &sizeOfListe) == 1) {
	  ...
	  &M_SizeOfResponseTimeRepartition);
    	...
    	// TODO: ERROR("Could not create table for response time repartition '%s'\n", P_listeStr);
    }
    delete [] listeInteger;
    listeInteger = NULL;
  }
...
int CStat::computeStat (E_Action P_action)
{
  switch (P_action)
    ...
    default :
      // TODO: ERROR("CStat::ComputeStat() - Unrecognized Action %d\n", P_action);
      return (-1);
    } /* end switch */
  return (0);
}
...
int CStat::findCounter(const char *counter, bool alloc) {
  ...
  if (!M_genericCounters) {
	  //TODO: ERROR("Could not allocate generic counters!\n");
  }
...
int CStat::findRtd(const char *name, bool start) {
  ...
  if (!M_rtdInfo) {
	  //TODO: ERROR("Could not allocate RTD info!\n");
  }
...
void CStat::validateRtds() {
  for (str_int_map::iterator it = rtd_started.begin(); it != rtd_started.end(); it++) {
    ...
    if (stopit == rtd_stopped.end() || !stopit->second) {
    	//TODO: ERROR("You have started Response Time Duration %s, but have never stopped it!", it->first.c_str());
    }
  }
}
