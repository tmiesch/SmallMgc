...
FileContents::FileContents(const char *fileName) {
  ...
  if (!inFile->good()) {
    // TODO: ERROR("Unable to open file %s", fileName);
  }
  ...
  if (NULL != strstr(line, "RANDOM")) {
      ...
      //TODO: ERROR("Unknown file type (valid values are RANDOM, SEQUENTIAL, and USER) for %s:%s\n", fileName, line);
  }
  ...
  if ((useprintf = strstr(line, "PRINTF"))) {
    ...
    if (*useprintf != '=') {
	// TODO: ERROR("Invalid file printf specification (requires =) for %s:%s\n", fileName, line);
    }
    useprintf++;
    char *endptr;
    virtualLines = strtoul(useprintf, &endptr, 0);
    if (*endptr && *endptr != '\r' && *endptr != '\n' && *endptr != ',') {
      // TODO: ERROR("Invalid file printf specification for (invalid end character '%c') %s:%s\n", *endptr, fileName, line);
    }
    if (virtualLines == 0) {
      // TODO: ERROR("A printf file must have at least one virtual line %s:%s\n", fileName, line);
    }
    printfFile = true;
  }
  ...
  if ((useprintf = strstr(line, "PRINTFOFFSET"))) {
    ...
    if (*useprintf != '=') {
	// TODO: ERROR("Invalid file PRINTFOFFSET specification (requires =) for %s:%s\n", fileName, line);
    }
    useprintf++;
    char *endptr;
    printfOffset = strtoul(useprintf, &endptr, 0);
    if (*endptr && *endptr != '\n' && *endptr != ',') {
      // TODO: ERROR("Invalid PRINTFOFFSET specification for (invalid end character '%c') %s:%s\n", *endptr, fileName, line);
    }
  }
  ...
  if ((useprintf = strstr(line, "PRINTFMULTIPLE"))) {
    ...
    if (*useprintf != '=') {
	// TODO: ERROR("Invalid PRINTFMULTIPLE specification (requires =) for %s:%s\n", fileName, line);
    }
    useprintf++;
    char *endptr;
  ...
  if (realLinesInFile == 0) {
    // TODO: ERROR("Input file has zero lines: %s\n", fileName);
  }
...
int FileContents::getField(int lineNum, int field, char *dest, int len) {
  ...
  if (curfield) {
      // TODO: WARNING("Field %d not found in the file %s", field, fileName);
      return 0;
  }
  ...
  if (x.length()) {
	...
	if (s[i] == '%') {
	  if (s[i + 1] == '%') {
	    ...
	    while (s[i] != 'd') {
	      if (i == l) {
		// TODO: ERROR("Invalid printf injection field (ran off end of line): %s", s);
	      }
	      if (!(isdigit(s[i]) || s[i] == '.' || s[i] == '-')) {
		// TODO: ERROR("Invalid printf injection field (only decimal values allowed '%c'): %s", s[i], s);
	      }
	      i++;
	    }
	    assert(s[i] == 'd');
...
int FileContents::nextLine(int userId) {
  switch(usage) {
	...
	// TODO: ERROR("%s has only %d lines, yet user %d was requested.", fileName, numLinesInFile, userId);
      }
      return userId - 1;
    default:
      // TODO: ERROR("Internal error: unknown file usage mode!");
      return -1;
  }
}
...
{
    /* TODO: WARNING("Line choosing strategy is [%s]. m_counter [%d] numLinesInFile [%d] realLinesInFile [%d]",
    ...
    */
    for (int i = 0; i < realLinesInFile && fileLines[i][0]; i++) {
       // TODO: WARNING("%s:%d reads [%s]", fileName, i, fileLines[i].c_str());
    }
}
...
int FileContents::lookup(char *key) {
  if (indexField == -1) {
    // TODO: ERROR("Invalid Index File: %s", fileName);
  }
  if (!indexMap) {
    // TODO: ERROR("Invalid Index File: %s", fileName);
  }
...
void FileContents::insert(char *value) {
  if (printfFile) {
    // TODO: ERROR("Can not insert or replace into a printf file: %s", fileName);
  }
...
void FileContents::replace(int line, char *value) {
  ...
  if (line >= realLinesInFile || line < 0) {
    // TODO: ERROR("Invalid line number (%d) for file: %s (%d lines)", line, fileName, realLinesInFile);
  }
  deIndex(line);
