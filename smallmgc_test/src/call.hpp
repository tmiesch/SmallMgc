...
class call : virtual public listener{
public:
  ...
  call(char *p_id, struct sockaddr_storage *dest);
  static call *add_call(int userId, bool ipv6, struct sockaddr_storage *dest);
  call(scenario * call_scenario, struct sockaddr_storage *dest, char * p_id, int userId, bool ipv6, bool isAutomatic, bool isInitCall);
...
private:
  ...
  void init(scenario * call_scenario, struct sockaddr_storage *dest, char * p_id, int userId, bool ipv6, bool isAutomatic, bool isInitCall);
...
private:
  ...
  bool executeMessage(class message *curmsg);
  T_ActionResult executeAction(char * msg, class message *message);
  void  extractSubMessage(char * msg, char * matchingString, char* result, bool case_indep,
							     int occurrence, bool headers);
  bool  rejectCall();
  double get_rhs(class CAction *currentAction);
  ...
  int   sendCmdMessage(class message *curmsg); // 3PCC
  ...
  void do_bookkeeping(class message *curmsg);
