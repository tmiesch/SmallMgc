...
bool CAction::compare(VariableTable *variableTable) {
  ...
  switch(M_comp) {
    ...
    case E_C_LEQ:
      return lhs <= rhs;
    default:
      Application::instance().logger().information("Internal error: Invalid comparison type" + NumberFormatter::format(M_comp));
...
void CAction::setRegExp(char *P_value) {
  ...
  if(errorCode != 0)
  {
    ...
    std::string expr(M_regularExpression);
    Application::instance().logger().information("recomp error : regular expression "
    		+ expr + "- error " + buffer);
  }
}
...
char *CAction::getRegularExpression() {
  if (!M_regExpSet) {
	  Application::instance().logger().information("Trying to get a regular expression for an action that does not have one! ");
  }
...
int CAction::executeRegExp(char* P_string, VariableTable *P_callVarTable)
{
  ...
  if (!M_regExpSet) {
    Application::instance().logger().information("Trying to perform regular expression match on action that does not have one!");
  }

  if (getNbSubVarId() > 9) {
    Application::instance().logger().information("You can only have nine sub expressions!");
  }
...
void CActions::setAction(CAction *P_action)
{
  ...
  if (!newActions) {
   Application::instance().logger().information("Could not allocate new action list.");
  }
  for (int i = 0; i < M_nbAction; i++) {
