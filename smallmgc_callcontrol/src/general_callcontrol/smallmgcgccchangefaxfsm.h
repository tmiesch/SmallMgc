...
#ifndef SMALLMGCGCCCHANGEFAXFSM_H_
#define SMALLMGCGCCCHANGEFAXFSM_H_

#include "smallmgcgccfsm.h"

class SmallMGC_GCC_ChangeFaxFsm : public SmallMGC_GCC_Fsm
{
public:
  static SmallMGC_GCC_ChangeFaxFsm* instance(class H248STACK_Database* database=0, class H248STACK_CallAdapter* adapter=0);
private :
   static SmallMGC_GCC_ChangeFaxFsm* s_instance;
   SmallMGC_GCC_ChangeFaxFsm(class H248STACK_Database* database, class H248STACK_CallAdapter* adapter):
     SmallMGC_GCC_Fsm(database, adapter)
     {

     };

  SmallMGC_GCC_ChangeFaxFsm();
  virtual ~SmallMGC_GCC_ChangeFaxFsm();
public:
...
#endif /* SMALLMGCGCCCHANGEFAXFSM_H_ */
