...
#ifndef SMALLMGCGCCSTARTEDFSM_H_
#define SMALLMGCGCCSTARTEDFSM_H_

#include "smallmgcgccfsm.h"

class SmallMGC_GCC_StartedFsm : public SmallMGC_GCC_Fsm
{
public:
  static SmallMGC_GCC_StartedFsm* instance(class H248STACK_Database* database=0, class H248STACK_CallAdapter* adapter=0);
private :
   static SmallMGC_GCC_StartedFsm* s_instance;
   SmallMGC_GCC_StartedFsm(class H248STACK_Database* database, class H248STACK_CallAdapter* adapter):
     SmallMGC_GCC_Fsm(database, adapter)
     {

     };

  SmallMGC_GCC_StartedFsm();
  virtual ~SmallMGC_GCC_StartedFsm();
public:
...
#endif /* SMALLMGCGCCSTARTEDFSM_H_ */
