...
#ifndef SMALLMGCNCCSTARTEDFSM_H_
#define SMALLMGCNCCSTARTEDFSM_H_

#include "smallmgcnccfsm.h"

class SmallMGC_NCC_StartedFsm : public SmallMgc_NCC_Fsm
{
public:
  static SmallMGC_NCC_StartedFsm* instance(class H248STACK_Database* database=0, class H248STACK_CallAdapter* adapter=0);
private :
   static SmallMGC_NCC_StartedFsm* s_instance;
   SmallMGC_NCC_StartedFsm(class H248STACK_Database* database, class H248STACK_CallAdapter* adapter):
     SmallMgc_NCC_Fsm(database, adapter)
     {

     };

  SmallMGC_NCC_StartedFsm();
  virtual ~SmallMGC_NCC_StartedFsm();
public:
